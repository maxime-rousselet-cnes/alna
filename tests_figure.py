...
def load_love_numbers_for_partials_plot(
    ...
    parameter_tab = array(
        object=[
            float(file.name.split(parameter)[1][1:-5])
            for file in test_path.glob("*" + parameter + "*")
        ],
        dtype=float,
    )
    parameter_tab.sort()
    ...
    for i_parameter, parameter_value in enumerate(parameter_tab):
        ...
        solid_earth_numerical_model = load_solid_earth_numerical_model(
            name=list(test_path.glob("*" + name + "*"))[0].name,
            path=test_path,
        )
    ...
    for ax_line, label, direction in zip(axes, "hlk", Direction):
        ...
        for ax, part in zip(ax_line, COMPLEX_PARTS):
            ...
            for i_period, period in enumerate(periods_tab):
                ...
                ax.semilogx(
                    parameter_tab[::FINITE_DIFFERENCES_SMOOTHER][:-1]
                    + diff(a=parameter_tab[::FINITE_DIFFERENCES_SMOOTHER]) / 2,
                    diff(
                        a=(
                            love_numbers[
                                i_period, direction.value, ::FINITE_DIFFERENCES_SMOOTHER
                            ].real
                            if part == "real"
                            else love_numbers[
                                i_period, direction.value, ::FINITE_DIFFERENCES_SMOOTHER
                    ...
                    / diff(parameter_tab[::FINITE_DIFFERENCES_SMOOTHER]),
